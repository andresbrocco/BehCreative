...
int pdPort = 3000;
int myPort = 3001;
Communication communication = new Communication("143.106.219.178" , pdPort, myPort, communication_rate);//"127.0.0.1"  "143.106.219.176" "143.106.219.182"
private color blue1 ;
...
private float jerkRange =  0.25; //quanto menor mais rápido aumenta a saturação de 0 até 1, sendo 0 colorido o tempo todo e 1 colorido somente quando o jerk está em 6 (máximo)
...
void updateParticleSystems(){
  for(Skeleton skeleton:scene.activeSkeletons.values()){
    ...
    if(scene.floor.isCalibrated){
        ...
        if(skeleton.joints[HAND_LEFT].standartDeviationNorm < this.jerkRange){
        ...
        if(skeleton.joints[HAND_LEFT].standartDeviationNorm > this.jerkRange && skeleton.joints[HAND_LEFT].standartDeviationNorm < 1){
          systemLeft.saturation = systemLeft.saturation + this.saturationIncrement;
          skeleton.joints[HAND_LEFT].saturation = systemLeft.saturation;
          skeleton.joints[HAND_LEFT].saturation = norm(skeleton.joints[HAND_LEFT].saturation,0,100);
        }
